...
class GroupChatScreen extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      creatorid: 0,
      recipientid: '0',
      recipient_group_id: parseInt(this.props.recipientid, 10),
      creatorname: '',
      recipientname: this.props.recipientname,
      re: this.props.re,
    }}
  liveChat = () => {
    fetch('http://triangle.bulme.at/~pi/MainAPI/Messages/GetGroupMessagelen.php',{
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        groupid: this.state.recipient_group_id,
      })
      ...
      .then((responseJson) => {
        if (responseJson != 'no results') {
          if (responseJson > this.state.messages.length) {
            ...
            Actions.refresh({
              key: this.state.re,
              recipientid: this.state.recipient_group_id,
  ...
  componentWillMount() {
    Realm.open({ // open connection
      ...
      fetch('http://triangle.bulme.at/~pi/MainAPI/Messages/GetGroupMessage.php',{
        method: 'POST',
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json',
        ...
        .then((responseJson) => {
          if (responseJson != 'no results') {
            ...
            this.setState({realm});
            for (let i = 0; i < this.state.realm.objects('messages').filtered('recipient_group_id =$0', this.state.recipient_group_id).length; i++) {
              let result = realm.objects('messages').filtered('recipient_group_id =$0 ', this.state.recipient_group_id).sorted('createdAt', true);
              this.setState({
                messages:
                  [...this.state.messages,
                    ...[
  ...
  onSend(messages = []) {
    ...
    const {recipient_group_id} = this.state; // Bei Gruppenchat
    fetch('http://triangle.bulme.at/~pi/MainAPI/Messages/SendGroupMessage.php', { //Bei Gruppenchat
      method: 'POST',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
      ...
      body: JSON.stringify({
        ...
        recipientname: this.state.recipientname,
        recipient_group_id: recipient_group_id,  // Bei Gruppenchat

      })
    }).done();

    fetch('http://triangle.bulme.at/~pi/MainAPI/Messages/GetGroupMessageID.php',
      {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
